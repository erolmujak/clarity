...
import { Check, Search, Pencil, Trash2, X, ChevronLeft } from "lucide-react"
...
export function ProfileSettings({ onClose }: ProfileSettingsProps) {
  ...
  async function handleSaveProfile() {
    ...
    try {
      ...
      setMessage({ type: "success", text: t("settings.profile_updated"), section: "profile" })
    } catch {
      setMessage({ type: "error", text: t("settings.profile_failed"), section: "profile" })
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleChangePassword() {
    if (newPassword.length < 6) {
      setMessage({ type: "error", text: t("settings.password_min"), section: "security" })
      return
    }
    if (newPassword !== confirmPassword) {
      setMessage({ type: "error", text: t("settings.passwords_mismatch"), section: "security" })
      return
    }
    ...
    try {
      ...
      setNewPassword("")
      setConfirmPassword("")
      setMessage({ type: "success", text: t("settings.password_updated"), section: "security" })
    } catch {
      setMessage({ type: "error", text: t("settings.password_failed"), section: "security" })
    } finally {
  ...
  return (
    <div className="min-h-screen bg-background overflow-y-auto">
      <div className="mx-auto max-w-xl px-5 py-10 sm:px-8">
        ...
        <h1 className="text-2xl font-semibold text-foreground mb-10">{t("settings.title")}</h1>
        ...
        <section className="mb-10">
          ...
          <SearchableList
            ...
            onSelect={(code) => setPreferredLanguage(code)}
        ...
        <section className="mb-10">
          ...
          <SearchableList
            ...
            onSelect={(code) => setGlobalCurrency(code)}
                        ...
                        <form
                          ...
                          onSubmit={async (e) => {
                            ...
                            if (editingTagValue.trim() && editingTagValue.trim().toLowerCase() !== tag) {
                              setSaving(true)
                              await renameTag(tag, editingTagValue.trim())
                              setSaving(false)
                            }
                            setEditingTag(null)
                            setEditingTagValue("")
